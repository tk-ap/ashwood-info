...
function derive(board,commands){
 ...
 set("routing",rows.some(r=>r.assignee)&&rows.some(r=>r.lane||r.status)?"VERIFIED":rows.length?"PARTIAL":"UNVERIFIED",rows.some(r=>r.assignee)&&rows.some(r=>r.lane||r.status)?"Canonical rows expose assignment plus lifecycle/routing state.":"Routing evidence is incomplete.","Surface routing decision, capacity reason and selected harness.");
